...
<template>
  <div :class="style.div">
    ...
    <div :class="style.actionsContainer">
      ...
      <ButtonIcon title="tick" :class="style.button" :the_id="`task-${task.id}-tick`" :icon="faCheck" :icon-style="style.icon" v-if="showTickAction" v-on:click="tick(task.id)"/>
      <ButtonIcon title="carry" :class="style.button" :the_id="`task-${task.id}-carry`" :icon="faArrowRight" :icon-style="style.icon" v-if="showCarryAction" v-on:click="carry(task.id)"/>
      <ButtonIcon title="remove" :class="style.button" :the_id="`task-${task.id}-remove`" :icon="faX" :icon-style="`${showCarryAction ? style.carryActionShown : ''} ${style.icon}`" v-if="showRemoveAction" v-on:click="remove(task.id)"/>
    </div>
  </div>
</template>
