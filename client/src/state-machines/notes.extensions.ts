...
export const getNextNoteListId = function (notesLists: NotesList[]) {
  let maxValue = 0;
...
export const ReadyToAddNotes = (value: StateValue) => {
  return (value !== NotesMachineCombinedStates.empty && value !== NotesMachineCombinedStates.readyToAddNotesLists);
};
...
export const notesHaveBeenCarried = function (context: { notes: Notes; }): boolean {
  return context.notes.filter((n) => !n.carried && !n.ticked).length === 0;
};

export const canCarryNote = function (note: Note): boolean {
  return note.page <= 1 && !note.ticked;
};
